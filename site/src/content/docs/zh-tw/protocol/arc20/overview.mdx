---
title: ARC-20
description: 一頁看完完整的 ARC-20 模型，從發行到配置再到安全邊界，並連結到每一部分的確切規則。
sidebar:
  order: 1
provenance:
  ...
  limitations:
    - 這是一份相容性指南，而不是獨立的共識規格。Atomicals 規格由程式碼定義。
---
...
<Aside type="danger" title="第六條是很多人付出代價才學會的">
...
<EntryCards
  cards={[
    { title: "配置視覺化工具", body: "建立交易模型，準確看到每個單位落在哪裡。", href: "/tools/allocation-visualizer/" },
    { title: "一致性向量", body: "支撐這些頁面上每個數字的確定性案例。", href: "/reference/conformance/" },
  ]}
/>
