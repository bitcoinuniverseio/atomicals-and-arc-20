...
public:
    ...
    enum class SigOpReturnStatus { NOT_CARRIER, CANONICAL, MALFORMED };

    SigOpReturnStatus ParseSigOpReturn(std::vector<uint8_t>& sig) const {
        sig.clear();
        if (empty() || *begin() != OP_RETURN) {
            return SigOpReturnStatus::NOT_CARRIER;
        }

        static constexpr uint8_t marker[] = {0x03, 0x73, 0x69, 0x67};
        if (size() == 1 || *(begin() + 1) != marker[0]) {
            return SigOpReturnStatus::NOT_CARRIER;
        }
        const size_t markerBytesPresent = std::min<size_t>(size() - 1, sizeof(marker));
        for (size_t index = 0; index < markerBytesPresent; ++index) {
            if (*(begin() + 1 + index) != marker[index]) {
                return SigOpReturnStatus::NOT_CARRIER;
            }
        }
        if (markerBytesPresent != sizeof(marker) || size() < 6) {
            return SigOpReturnStatus::MALFORMED;
        }

        opcodetype opcode;
        std::vector<uint8_t> item;
        CScript::const_iterator it = begin() + 5;
        if (!GetOp(it, opcode, item) || opcode > OP_PUSHDATA4 || !CheckMinimalPush(item, opcode) || it != end()) {
            return SigOpReturnStatus::MALFORMED;
        }
        // Signature syntax and cryptographic validity are checked by the
        // interpreter. This parser only establishes a canonical carrier.
        sig = item;
        return SigOpReturnStatus::CANONICAL;
    }
